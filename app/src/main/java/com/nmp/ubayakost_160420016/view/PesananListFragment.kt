    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        activity?.navView?.getHeaderView(0).let {
            ...
            it?.btnLogOutHeader?.setOnClickListener {
                ...
                val action = PesananListFragmentDirections.actionPesananToLogin()
                Navigation.findNavController(view).navigate(action)
            }

            it?.imgAvatar?.setOnClickListener { v2 ->
                activity?.drawerLayout?.closeDrawer(GravityCompat.START)
                val action = PesananListFragmentDirections.actionPesananToProfile("order")
                Navigation.findNavController(view).navigate(action)
            }
        }
        ...
        activity?.onBackPressedDispatcher?.addCallback(requireActivity(), object : OnBackPressedCallback(true) {
            override fun handleOnBackPressed() {
                val action = PesananListFragmentDirections.actionPesananToHome()
                Navigation.findNavController(view).navigate(action)
            }
        })
    }
